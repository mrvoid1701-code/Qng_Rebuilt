...
We present a derivation of Planck's constant `ℏ` from a discrete
graph-based substrate (Quantum Node Gravity, QNG) using only the
substrate's geometric and coupling parameters together with a single
physical principle: **cosmological stability**, requiring vacuum energy
density to vanish. The derived value
