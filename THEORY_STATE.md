...
| 13 | **Scale separation Planck → MeV/GeV** | **NEW open-HIGH** (2026-04-25) | 22-order tension between unit-bridge (Planck) and observed (MeV/GeV) particle masses. DER-QNG-038 baryon ladder retracted (Gap 14 finite-size + Gap 13 calibration). Blocks particle ID, mass derivations. Classical α-running ruled out (CPU-141). **2026-06-01 REFRAME (demo Phase 10, demo-phase10-vortex-node-rg-v1): the vortex↔node coarse-graining (RG) shows TOPOLOGY is RG-invariant (winding preserved exactly across 5 block-spin levels) → scale-free (explains why charges/Eightfold-Way/J-band were computable), while MASS is RG-flowing → scale-dependent (why blocked). Gap 13 = an RG DISTANCE (~log₂(10²²)≈73 block steps), reframed as dimensional transmutation, NOT a topological-sector mystery. Forward attack: measure the dimensionful RG flow of the full v8/v13 dynamics (look for a near-marginal coupling = exponential hierarchy from O(1) input). Still open, now located.** **2026-06-01 ATTACK (demo Phase 11, demo-phase11-dim-transmutation-v1): the HADRON-scale part is substantially explained by DIMENSIONAL TRANSMUTATION of the confining edge-SU(3) coupling (Phase 3): Lambda = M_P·exp(-2π/(b0·α_s(M_P))) with b0=9 gives the observed 19.8-order M_P/Λ_QCD from α_s(M_P)=0.0153 — an O(0.01) input (= SM strong coupling extrapolated to M_P). The 22 orders are NOT fine-tuning, they are the exponential of a moderate coupling (same as QCD). Hadron mass scale = dimensional transmutation. REMAINING open: ℏ_QNG (axiomatic) + lepton/quark Yukawa masses (separate sector); α_s(M_P) input = Gap 17.** |
...
| 17 | Fine structure α_fine | **NEW open** (2026-04-25) | e is INPUT to v12, not derived. Same status as α_fine in QED (input only). **2026-06-01 generalized to "gauge group choice"**: the Hodge no-go (DER-QNG-101) forces carriers to be edge-valued but NOT which group (why U(1)×SU(2)×SU(3)) — Gap-17 generalized. |
| 18 | **Non-abelian gauge on edges (weak/strong)** | **NEW: edges CAN host (2026-06-01, demo Phase 3)** | demo `t_phase3_su2_edges.py` (audit demo-phase3-su2-edges-v1): SU(2) quaternion links + Wilson Metropolis → gauge-invariant (exact), plaquette matches β/4, **area-law confinement** (lnW22/lnW11=4.03 at strong coupling). QNG edges host a genuine confining non-abelian gauge theory. PURE-GAUGE only. |
| 19 | **Non-abelian MATTER multiplet** | **NEW open-HIGH (2026-06-01, demo Phase 4) — needs v13** | The hard wall. 4a (audit demo-phase4a-custodial-v1): NO custodial (σ_g,σ_m) SU(2) — two real scalars can't form an SU(2) doublet (group theory + channel-F breaks even SO(2)). 4b (demo-phase4b-v13-matter-v1): a complex node doublet ψ∈ℂ² coupled to SU(2) links IS consistent (gauge-invariant exact; W rotates isospin) — **v13 door works but is genuinely NEW node ontology**. Blocks quarks/leptons. **Phase 5 (demo-phase5-v13-skyrmion-v1): v13's SU(2) field U(x) hosts a GENUINE B=1 baryon** (hedgehog B→1 in π₃; U(1) phase gives B=0 — the φ-ring is only a baby-Skyrmion in π₁), AND gives the pion TRIPLET. **One v13 fix closes 3 gaps: weak-matter doublet + genuine baryon + pion triplet.** |
| 20 | **Chirality (parity violation)** | **SURMOUNTABLE (2026-06-01, demo Phase 9)** | 4c (demo-phase4c): naive lattice Dirac → 16 doublers (Nielsen-Ninomiya); Wilson breaks chirality. **Phase 9 (demo-phase9-v14-domain-wall-v1): the domain-wall mechanism gives one chiral mode per wall (chirality ±1.0 exact, doublers gapped) — circumvents Nielsen-Ninomiya.** v14 chiral fermions (quarks/leptons) NOT blocked in principle: known lattice-chiral tech (domain-wall/overlap/Ginsparg-Wilson), a real build not a no-go. **The single genuinely hard remaining wall is the ABSOLUTE SCALE (ℏ program + Gap 13), not chirality.** |

**2026-06-01 demo-theory edge/node ontology (DER-QNG-101 + Phase 1-4)**: FORCES live on EDGES (photon U(1) confirmed v12/Ph2; W/Z/gluons+confinement SU(2) Ph3; graviton rank-2 E8), MATTER/MASS lives on NODES (σ_m rings = topological solitons, mass=volume charge per demo E4). **Skyrme reframing**: QNG rings are baryon-like topological solitons (baryon#=winding=M_ring; J/I from collective quantization explains even/odd-R→I=½/3/2 and absent Roper) — NOT elementary fermions. Layered map: v8 (mass) → v11 (graviton edge) → v12 (photon edge) → **v13 (SU(2)/SU(3) edge + complex node multiplet; Skyrmion baryons)** → **v14 (lattice-chiral fermions = quarks/leptons)**. See `demo-theory/PHASE3-edges-findings.md`, `PHASE4-particles-reattack.md`. |
...
| 12 | Tensor graviton ontology | **NEW open-HIGH** (2026-04-24, QG Phase B) — CPU-118 confirmed sigma_g wave is massless scalar with omega²=c_g²k², c_g=c_phi exactly (DER-QNG-042 §3.3 protection). But sigma_g is a SCALAR field per-node → its quantum is spin-0, NOT the GR spin-2 graviton. einstein-mind verdict (DER-QNG-069, `qng-gap12-tensor-graviton-v1.md`): "genuine structural gap, needs new ontology". Phenomenology PASS (DER-QNG-068) comes from scalar Newtonian-gauge metric matching GR at post-Newtonian order; GW tensor modes (h+, h_x) require either (a) emergent tensor from ring-background decomposition (GPU-047 pending), or (b) rank-2 edge primitive (parallels hbar-edge program finding that scalar edges were structurally insufficient). Open-HIGH because observationally testable at leading order via LIGO/Virgo polarization constraints. **2026-06-01 UPDATE (DER-QNG-101, Hodge no-go)**: the node-vs-edge ambiguity is RESOLVED toward **edge-valued** — node scalars cannot carry transverse modes (`curl(d scalar)=0`, demo E5/E7 machine-precision), so the carrier is forced to be directional/edge-valued; demo E8 confirms a rank-2 carrier hosts exactly 2 TT polarizations at c_phi (kinematic). Gap 12 now reduces to its **dynamical** part only (does the substrate produce the edge rank-2 dynamics — GPU-047). v12 photon A_ij (DER-QNG-076) is the same lesson at spin-1, now upgraded from arbitrary to forced. **2026-06-01 Phase 16 (demo-phase16-dynamical-graviton-v1) DYNAMICAL_GRAVITON_ON_EDGES**: the edge rank-2 object carries a genuine DYNAMICAL graviton — linearized Riemann curvature is **diffeomorphism-gauge-invariant** (h→h+∂ξ+∂ξ leaves it unchanged, 4.5e-16, machine precision — the defining graviton property E8 didn't test), gauge modes carry zero TT content, exactly 2 physical dof. **Gap 12 splits: carrier+consistency DONE (E8 kinematic + Phase 16 dynamical/gauge-invariant); remaining CORE = derive the Fierz-Pauli action by coarse-graining the substrate** (the prerequisite for f_g→α→parameter-free proton mass, Phases 14/15). **2026-06-01 Phases 17-20 advance the core: Phase 17 linearized coefficient substrate-derived (μ_h=β_g μ_φ/β_φ matches GR to 15%; target corrected edge-not-σ_g); Phase 18 Sakharov heat-kernel gives ~4% of G as FULL-covariant-R (rigorous nonlinear, partial); Phase 20 NONLINEAR_COMPLETION_IS_REGGE — the nonlinear completion is Regge calculus (deficit-angle=full nonlinear curvature, Σδ=4π to 1e-14 Gauss-Bonnet; δ/area→K; deficit nonlinear in edge-lengths; Regge→∫√gR rigorous). Remaining piece now BOUNDED: derive the Regge measure (hinge areas + 1/8πG coupling, 15%-matched) from the substrate — "derive a lattice measure," not "find the nonlinear action."** |
